...
exports.getCategoryProduct = (req, res) => {
  try {
    ...
    Category.findOne({ slug: root }, '_id')
      .exec((error, categ) => {
        ...
        if (error) {
          ...
          Category.findOne({ parentId: categ._id, slug: sub },"_id")
            .exec((err, cat) => {
              if (err) res.status(400).json({ err });
              else {
                //find the child category of the sub category
                Category.findOne({ parentId: cat._id, slug: category },"_id")
                  .exec((err, gory) => {
                    if (err) {
